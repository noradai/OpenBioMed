...
class DTIModel(nn.Module):
    def __init__(self, config):
        ...
        self.pred_head = MLP(config["pred_head"], self.drug_encoder.output_dim + self.protein_encoder.output_dim, 1)
...
class DeepEIK4DTI(nn.Module):
    def __init__(self, config):
        ...
        self.text_project = nn.Sequential(
            nn.Linear(self.text_encoder.hidden_dim, self.projection_dim),
            nn.Dropout(config["projection_dropout"])
        )
        ...
        self.pred_head = MLP(config["pred_head"], self.structure_hidden_dim + 2 * self.projection_dim, 1)
    ...
    def forward(self, drug, protein):
        ...
        h_kg = self.kg_project(torch.cat((drug["kg"], protein["kg"])))
        h_text = self.text_encoder(**drug["text"]).last_hidden_state
        if self.use_attention:
